...
/**
 * 부서 검색 모달 트리거 + 선택된 부서명 표시 입력창.
 * - value: id (string) — 폼 값으로 그대로 사용 가능
 * - 라벨 표시: 캐시된 useGetDepartmentsQuery (전체 부서 목록) 에서 lookup
...
export default function DepartmentSelectField({
  ...
  // 선택된 부서의 표시 라벨 — 모달과 별개로 캐시된 reference 쿼리에서 lookup.
  const { data: depts = [] } = useGetDepartmentsQuery();
  const selectedLabel = depts.find((d) => String(d.id) === value)?.name ?? '';

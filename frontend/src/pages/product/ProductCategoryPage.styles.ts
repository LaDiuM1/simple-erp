...
interface CategoryRowStyleProps {
  isDragging: boolean;
...
})<CategoryRowStyleProps>(({ theme, isDragging, isDropTarget, canWrite }) => ({
  paddingTop: '0.5rem',
...
export const OrderBadge = styled(Box)(({ theme }) => ({
  minWidth: 28,
  height: 28,
  display: 'inline-flex',
  alignItems: 'center',
...
/** 행 우측의 수정 / 삭제 아이콘 묶음 — 행 여백을 흡수해 우측 정렬. */
